...
func testLoadConfig(t *testing.T, hasFile, force bool) {
	...
	if hasFile && !force {
		...
		if diff := cmp.Diff(expectConfig, config); diff != "" {
...
func testSetParam(t *testing.T, paramName, param, expectedMessage string) {
	...
	case paramName == CurrentContextKey:
		...
		if param == newContext {
			if diff := cmp.Diff(emptySdctlContext(), config.SdctlContexts[config.CurrentContext]); diff != "" {
				t.Errorf("expected='%v', actual='%v'", emptySdctlContext(), config.SdctlContexts[config.CurrentContext])
			}
		} else {
			if diff := cmp.Diff(config.SdctlContexts[param], config.SdctlContexts[config.CurrentContext]); diff != "" {
				t.Errorf("expected='%v', actual='%v'", config.SdctlContexts[param], config.SdctlContexts[config.CurrentContext])
			}
		}
	default:
